...
Vector3 PhysicsSystem::GetGravity() { return gravity; };
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (a.GetTag() == Tag::_Trigger || b.GetTag() == Tag::_Trigger)
		return;
	...
	for (auto i = first; i != last; ++i) {
		...
		if (applyGravity && inverseMass > 0) {
			if (!object->IsSleeping()) {
				accel += (*i)->CanUseOfCustomGravity() ? (*i)->GetCustomGravity() : gravity;
...
position and
orientation. It may be called multiple times
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		float frameLinearDamping = 1.0f - (object->GetTotalFriction() * dt);
		if (object == nullptr) {
			continue;
		...
		//Attempt to apply sleep
		if (object->GetInverseMass() != 0) {
			object->RegisterSquareDisplacemet((linearVel).LengthSquared());
			if (elapsedFramesForSleepCheck >= checkForSleepInFrames) {
				object->UpdateSleep();
				elapsedFramesForSleepCheck = 0;
			}
			elapsedFramesForSleepCheck++;
		}
		if (!object->IsSleeping())
		...
		float frameAngularDamping = 1.0f - (object->GetTotalFriction() * dt);
...
void PhysicsSystem::ResolveSpringCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float F = (-k * x);
	Vector3 force = p.normal * F;
	a.GetPhysicsObject()->AddForce(force);
	b.GetPhysicsObject()->AddForce(-force);
}
