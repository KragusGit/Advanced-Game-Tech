...
void TutorialGame::UpdateGame(float dt) {
	...
	if (followPlayer) {
		Debug::Print("(F)ollow on", Vector2(5, 95));
	}
	else {
		Debug::Print("(F)ollow on", Vector2(5, 95));
	}
	...
	if (lockedObject != nullptr && !followPlayer) {
	...
	if (followPlayer) {
		Vector3 objPos = playerObj->GetTransform().GetPosition();
		Vector3 camPos = objPos + ((playerObj->GetTransform().GetOrientation() * Vector3(0, 50, 50)));
		...
		if (camMoveDir.LengthSquared() > 1)
			world->GetMainCamera()->SetPosition(world->GetMainCamera()->GetPosition() + ((camMoveDir.Normalised() * (50 / camMoveDir.LengthSquared())) * dt));
		else
			world->GetMainCamera()->SetPosition(camPos);
		float curYaw = world->GetMainCamera()->GetYaw();
...
*/
GameObject* TutorialGame::AddFloorToWorld(const Vector3& position, float friction = 1) {
...
void TutorialGame::InitDefaultFloor() {
	...
	//AddFloorToWorld(Vector3(200, -2, 0), 3);
	//AddFloorToWorld(Vector3(0, -2, 200), 5);
}

void TutorialGame::InitGameExamples() {
	//AddPlayerToWorld(Vector3(0, 5, 0));
	playerObj = AddMainPlayerToWorld(Vector3(0, 5, -20));
	AddCubeToWorld(Vector3(0, 6, 0), Vector3(3, 2.5, 3), 0);
...
PlayerObject* TutorialGame::AddMainPlayerToWorld(const Vector3& position) {
	float meshSize = 3.0f;
	float inverseMass = 0.5f;

	PlayerObject* character = new PlayerObject();

	OBBVolume* volume = new OBBVolume(Vector3(meshSize * 0.5f, meshSize * 0.5f, meshSize * 0.5f));
